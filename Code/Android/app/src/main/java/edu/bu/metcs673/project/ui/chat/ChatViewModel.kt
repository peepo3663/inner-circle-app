...
class ChatViewModel(application: Application) : AndroidViewModel(application) {
    ...
    private fun fetchMessages() {
        ...
        chatsRef.whereArrayContains("users", currentUserId).get()
            .addOnSuccessListener { querySnapshot ->
                for (document in querySnapshot.documents) {
                    currentChatId = document.id
                    document.reference.collection("messages").orderBy("createdAt").get().addOnSuccessListener { messages ->
    ...
    fun insert(userMessage: MessageModel) {
        ...
        chatsRef.document(currentChatId!!).collection("messages").add(messageInChat).addOnSuccessListener { fetchMessages() }
    }
}
